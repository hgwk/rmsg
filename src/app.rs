...
impl RelayApp {
    pub fn new(store: Arc<MessageStore>, kp: Arc<KeyPair>) -> Self {
        ...
        std::thread::spawn(move || {
            rt.block_on(async {
            let mut node = match P2PNode::new(event_tx, None) { Ok(n) => n, Err(e) => { log::error!("P2P init: {}", e); return; } };
...
impl RelayApp {
    fn drain_p2p_events(&mut self) {
        while let Ok(event) = self.p2p_events.try_recv() {
            match event {
                P2PEvent::Message { data, .. } => {
                    if let Some(ref mut session) = self.session {
                        if let Ok(wire) = serde_json::from_slice::<WireMessage>(&data) {
                            if matches!(wire, WireMessage::Control(ControlMessage::Typing { .. })) {
                        ...
                        for ev in &events {
                            if !ev.mine && ev.from != "system" {
                                self.connection_status = format!("Chatting with {}", ev.from);
                            }
                        }
                ...
                P2PEvent::Subscribed => {
                    ...
                    if self.send_hs {
                        ...
                        if let Some(ref session) = self.session {
                            ...
                            let _ = self.p2p_cmd.send(P2PCmd::Send(
                                rid.clone(),
                                session.mk_handshake().into_bytes(),
                            ));
                            let _ = self
                                .p2p_cmd
                                .send(P2PCmd::Send(rid, session.build_want_since().into_bytes()));
                        }
                    }
                }
                ...
                P2PEvent::PeerAddress(_) | P2PEvent::Listening(_) => {}
    ...
    fn show_lobby(&mut self, ui: &mut egui::Ui) {
        ui.vertical_centered(|ui| {
            ...
            ui.add_space(4.0);
            ui.label(
                egui::RichText::new("No signup. No phone. No server.").color(egui::Color32::GRAY),
            );
            ui.add_space(2.0);
            let net_dot = if self.peers.is_empty() { "○" } else { "●" };
            let net_color = if self.peers.is_empty() {
        ...
        if self.rooms.is_empty() {
            ui.vertical_centered(|ui| {
                ...
                ui.add_space(4.0);
                ui.label(
                    egui::RichText::new("Create a room and share the code with your friend.")
            ...
            egui::ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    for room in &self.rooms {
                        if ui.button(&room.id).clicked() {
                            selected = Some(room.id.clone());
                        }
                    }
                });
        ...
        ui.vertical_centered(|ui| {
            if ui
                .button(egui::RichText::new("+ Create new room").size(14.0))
                .clicked()
            {
            ...
            ui.horizontal(|ui| {
                ...
                let resp = ui.add_sized(
                    ...
                    egui::TextEdit::singleline(&mut self.join_input)
                        .hint_text("paste room code..."),
                );
                if resp.lost_focus()
                    && ui.input(|i| i.key_pressed(egui::Key::Enter))
                    && !self.join_input.is_empty()
                {
                    let rid = self.join_input.trim().to_string();
    ...
    fn enter_room(&mut self, room_id: String) {
        ...
        self.messages.push(ChatEvent {
            id: uuid::Uuid::new_v4().to_string(),
            from: "system".into(),
            text: format!(
                "Room: {} — copied to clipboard. Share with your peer.",
                room_id
            ),
            ts: now_ms(),
            mine: false,
    ...
    fn show_chat(&mut self, ui: &mut egui::Ui, room_id: &str) {
        ui.horizontal(|ui| {
            ...
            ui.heading(if self.room_alias != *room_id {
                &self.room_alias
            } else {
                room_id
            });
        ...
        let (color, text) = if ready {
            (
                egui::Color32::GREEN,
                if self.connection_status.contains("Chatting") {
                    format!("🔒 End-to-end encrypted — {}", self.connection_status)
                } else {
                    "🔒 End-to-end encrypted".into()
                },
            )
        } else if self.connection_status.is_empty() {
            (egui::Color32::YELLOW, "⏳ Waiting for peer...".into())
        } else {
            (egui::Color32::YELLOW, self.connection_status.clone())
        };
        ui.colored_label(color, &text);
        ui.horizontal(|ui| {
            if ui.small_button("📋 Copy ID").clicked() {
                self.pending_clipboard = Some(room_id.to_string());
            }
            ui.label("Alias:");
            ui.add_sized(
                [120.0, 18.0],
                egui::TextEdit::singleline(&mut self.room_alias_input).hint_text("optional"),
            );
        });
        ui.separator();
            ...
            .show(ui, |ui| {
                if self.messages.is_empty() {
                    ui.vertical_centered(|ui| {
                        ...
                        ui.label(
                            egui::RichText::new(
                                "Share the room code with your peer to start chatting.",
                            )
                            .color(egui::Color32::DARK_GRAY)
                            .size(11.0),
                        );
                ...
                for msg in &self.messages {
                    ...
                    if msg.from == "system" {
                        ...
                        ui.with_layout(egui::Layout::right_to_left(egui::Align::Max), |ui| {
                            ui.horizontal(|ui| {
                                ui.label(
                                    egui::RichText::new(format_time(msg.ts))
                                        .color(egui::Color32::DARK_GRAY)
                                        .size(10.0),
                                );
        ...
        ui.horizontal(|ui| {
            ...
            if resp.changed() {
                ...
                if now.duration_since(self.last_typing_sent).as_millis() > 500 {
                    self.last_typing_sent = now;
                    let tm = serde_json::to_string(&WireMessage::Control(ControlMessage::Typing {
                    ...
                    if let Some(ref session) = self.session {
                        let _ = self
                            .p2p_cmd
                            .send(P2PCmd::Send(session.room_id().to_string(), tm.into_bytes()));
                    }
                }
            }
                ...
                if let Some(ref mut session) = self.session {
                    if session.is_ready() {
                        if let Ok(encoded) = session.encrypt_and_encode(&text) {
                            let _ = self
                                .p2p_cmd
                                .send(P2PCmd::Send(room_id.to_string(), encoded.into_bytes()));
                        }
                    }
                }
                self.messages.push(ChatEvent {
