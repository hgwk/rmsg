...
impl ChatSession {
    pub fn new(room_id: String, user_id: String, store: Arc<MessageStore>, kp: KeyPair) -> Self {
        ...
        Self { room_id, user_id, seq: 0, kp, peer_pk: None, cs, ready: false, seen: HashSet::new(), store }
    }

    pub fn room_id(&self) -> &str { &self.room_id }
    pub fn is_ready(&self) -> bool { self.ready }
    #[allow(dead_code)] pub fn current_seq(&self) -> u64 { self.seq }
    ...
    pub fn handle_raw(&mut self, raw: &[u8]) -> Vec<ChatEvent> {
        let wire: WireMessage = match serde_json::from_slice(raw) { Ok(w) => w, Err(_) => return vec![] };
        self.process_wire(wire)
    }

    pub fn build_want_since(&self) -> String {
        let last_ts = self.store.last_ts(&self.room_id).unwrap_or_default().unwrap_or(0);
        serde_json::to_string(&WireMessage::Control(ControlMessage::WantSince {
            from: self.user_id.clone(), since: last_ts, sid: self.user_id.clone(),
    ...
    fn process_wire(&mut self, wire: WireMessage) -> Vec<ChatEvent> {
        ...
        match wire {
            WireMessage::Chat(msg) => {
                if msg.from == self.user_id || self.seen.contains(&msg.id) { return events; }
                self.seen.insert(msg.id.clone());
                if let (Ok(c), Ok(n)) = (base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &msg.ct), base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &msg.nonce)) {
                    if n.len() == 12 {
                        let mut nonce = [0u8; 12]; nonce.copy_from_slice(&n);
                        if let Ok(plain) = self.cs.decrypt(&c, &nonce, msg.seq) {
                            let text = String::from_utf8_lossy(&plain).to_string();
                            self.store.save(&StoredMessage { id: msg.id.clone(), room_id: self.room_id.clone(), from_id: msg.from.clone(), text: text.clone(), ts: msg.ts }).ok();
                            events.push(ChatEvent { id: msg.id, from: msg.from, text, ts: msg.ts, mine: false });
                        }
                    }
                }
            }
            WireMessage::Control(ctrl) => match ctrl {
                ControlMessage::PublicKey { from, pk, .. } => {
                    if from != self.user_id && !self.ready {
                        if let Ok(bytes) = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &pk) {
                            if bytes.len() == 32 {
                                let mut arr = [0u8; 32]; arr.copy_from_slice(&bytes);
                                self.set_peer_pk(&arr).ok();
                                events.push(ChatEvent { id: uuid::Uuid::new_v4().to_string(), from: "system".into(), text: "Encryption established".into(), ts: chrono_now(), mine: false });
                            }
                        }
                    }
                }
                ControlMessage::HistoryChunk { from, items, .. } => {
                    if from == self.user_id { return events; }
                    for item in items {
                        if let (Ok(c), Ok(n)) = (base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &item.ct), base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &item.nonce)) {
                            if n.len() == 12 {
                                let mut nonce = [0u8; 12]; nonce.copy_from_slice(&n);
                                if let Ok(plain) = self.cs.decrypt(&c, &nonce, item.seq) {
                                    let text = String::from_utf8_lossy(&plain).to_string();
                                    self.store.save(&StoredMessage { id: item.id.clone(), room_id: self.room_id.clone(), from_id: item.from.clone(), text: text.clone(), ts: item.ts }).ok();
                                    events.push(ChatEvent { id: item.id, from: item.from, text, ts: item.ts, mine: false });
                                }
                            }
                        }
                    }
    ...
    pub fn encrypt_and_encode(&mut self, text: &str) -> Result<String, String> {
        ...
        let enc = self.cs.encrypt(text.as_bytes(), self.seq).map_err(|e| e.to_string())?;
    ...
    pub fn build_history_chunk(&mut self, since: i64) -> String {
        ...
        for m in &missing {
            self.seq += 1;
            let enc = self.cs.encrypt(m.text.as_bytes(), self.seq).map(|e| (e.ct, e.nonce)).unwrap_or_default();
        ...
        serde_json::to_string(&WireMessage::Control(ControlMessage::HistoryChunk {
            from: self.user_id.clone(), since, items, sid: self.user_id.clone(),
